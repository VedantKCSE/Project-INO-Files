  ...
  if (WiFi.status() == WL_CONNECTED) {
    ...
    if (httpResponseCode > 0) {
      ...
      if (schedule.size() > 0) {
        ...
        char currentAmPm[3];
        sscanf(currentTimeStr, "%d:%d %s", &currentHour, &currentMinute, currentAmPm);
        if (strcmp(currentAmPm, "PM") == 0 && currentHour != 12) currentHour += 12;
        if (strcmp(currentAmPm, "AM") == 0 && currentHour == 12) currentHour = 0;
        ...
        for (JsonObject lecture : schedule) {
          ...
          int lectureHour, lectureMinute;
          char lectureAmPm[3];
          sscanf(start_time, "%d:%d %s", &lectureHour, &lectureMinute, lectureAmPm);
          if (strcmp(lectureAmPm, "PM") == 0 && lectureHour != 12) lectureHour += 12;
          if (strcmp(lectureAmPm, "AM") == 0 && lectureHour == 12) lectureHour = 0;
          int lectureMinutes = lectureHour * 60 + lectureMinute;

          if (currentMinutes < lectureMinutes) {
        ...
        if (!nextLectureFound) {
          ...
          tft.println("End of Schedule");
        }
      } else {
        tft.setCursor(10, 140);
        tft.println("No Schedule Available");
      }
